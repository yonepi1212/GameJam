...
/* TrinityTimelineToolInspector2.cs
    TimeLinePlayerを設定するためのカスタムエディタ拡張
    (no scriptable object)
*/
[CustomEditor((typeof(TrinityTimelineTool2)))]
public class TrinityTimelineToolInspector2 : Editor
{
    #region Declaration
    private TrinityTimelineTool2 _timeLine;
    #endregion

    #region Editor
    private void OnEnable()
    {
        _timeLine = target as TrinityTimelineTool2;
    }
    ...
    private void AddEventField()
    {
        _timeLine.Events.ForEach((trinityEvent) =>
        {
            ...
            EditorGUILayout.LabelField(trinityEvent.FireEvent.GetPersistentMethodName(0), guiStyle);
